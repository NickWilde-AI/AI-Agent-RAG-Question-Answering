...
**Agent 执行过程**：`GET /research/jobs/{job_id}/events` 通过 SSE 推送规划、检索、校验、重试和报告事件，并支持 `Last-Event-ID` 续传。前端展示的是可审计执行事件和工具结果，不暴露模型私有 chain-of-thought。
...
| **千问 VL 页面解析** | 扫描页、表格、图表、流程图转结构化 Markdown | `bash scripts/one_click_demo.sh --qwen`，`RAG_VISION_PARSER_MODEL` |
