...
- **可选配置**：OpenAI-compatible、Redis、Milvus、ColPali、VLM、LangGraph；均不是轻量演示的启动前提。
...
- 轻量 demo 可关闭 vLLM、ColPali、Milvus 和 Redis，在普通开发机运行。
- GPU 型号、显存、CPU、带宽和副本数必须根据实际模型、页面量、并发、延迟目标和压测结果确定；本仓库不提供未经验证的“推荐生产规格”。
- 当前默认 SQLite + 进程内 dispatcher 适合单机演示，不代表已经验证大规模并发或千万 DAU。
...
| 限流 | `/ask`、研究提交和上传按客户端令牌桶限流，429 | `RAG_ENABLE_RATE_LIMIT`，`src/middleware_ops.py` |
