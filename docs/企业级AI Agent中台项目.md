...
本阶段首批落地四个功能，不加入会议纪要和机器人控制。需要强调的是：**这四个 Skill 是验证中台抽象成立的首批示范，不是能力上限。** 中台的核心价值在于——任何企业场景（考勤、审批、工单、报销、IT 资产、制度问答……）都能按同一套 Skill 契约接入，共用底座的检索、权限、监控、评测能力，平台层无需改动。公司内部有多少个需要 AI 辅助的场景，就能挂多少个 Skill。
...
> 扩展示例：新增一个考勤 Skill 只需实现 `SkillSpec`（声明能力与风险等级）、写业务逻辑（内部可调 RAG 检索或对接考勤系统 API）、按 `SkillResult` 契约返回——底座、权限、限流、监控、Trace、评测和前端都无需改动。
...
核心心智模型：**RAG 检索不是四个功能之一，而是所有 Skill 共享的底座能力层。** 中台对外提供两样东西——一是底座能力（文档解析、页级检索、VLM 页图理解、Verifier、证据页），二是统一契约（`SkillSpec` 声明能力、`SkillResult` 规范输出）。业务 Skill 只需按契约接入：内部调用底座检索、写自己的领域逻辑、按统一结构返回。业务方不需要了解底座检索如何实现，就像用水电只需接上插座，不必懂供电原理。这套"底座 + 契约"的分层，正是四个场景能共用一套系统、且未来能低成本扩展的根本原因。
