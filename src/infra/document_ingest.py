"""
document_ingest.py — 多格式入库编排：扫描目录 → 统一转为 PDF（必要时）→ `ingest_pdf_with_pymupdf`

================================================================================
【在「简历第一条」里的位置】
================================================================================
- 离线批处理：把 Office/表格等纳入与 PDF 同一套 **Page** 抽象，供索引脚本写入 JSON / 向量库。
- 与在线 `QAEngine.ask` 解耦；面试可说「多格式文档进同一页级索引管线」。

================================================================================
【类比 Android】
================================================================================
- 像 **ContentResolver + 各 Type 的 DocumentProvider**：对调用方只暴露「拿到 Page 列表」，内部处理 docx/pptx 等差异。

================================================================================
【从 Java/Kotlin 读 Python】
================================================================================
- `SUPPORTED_EXTENSIONS = {".pdf", ...}`：**set 字面量**，成员测试 `ext in SUPPORTED_EXTENSIONS` 为 O(1)。
- `subprocess.run` / `tempfile`：调外部 LibreOffice 等可执行文件，类似 `ProcessBuilder`。
- `warnings.warn`：非致命告警，不中断流程。
