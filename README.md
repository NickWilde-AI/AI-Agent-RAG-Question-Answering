...
如需远程访问，请开启仓库内置 JWT + Workspace ACL，并在网关配置 TLS、请求大小限制和租户隔离后再开放端口；企业 SSO/OIDC 仍由组织身份平台集成，不应裸露到公网。
...
默认保持本地演示的无登录体验。设置 `RAG_ENABLE_AUTH=true` 后，Workspace 接口将校验 HS256 Bearer JWT，并按 `user:<sub>` / `group:<groups>` 执行 `viewer`、`editor`、`owner` ACL；旧的 Workspace 不会被默认公开。生产可由 API Gateway/OIDC 签发同类 claims。

项目同时提供标准 MCP stdio Server，对外暴露页级检索、完整 QA、三类 QA 工具与研究计划：

```bash
python scripts/mcp_server.py
```

MCP Client 的 command 填写仓库虚拟环境中的 Python，args 填写上述脚本绝对路径即可。复杂研究可设置 `RAG_ENABLE_RESEARCH_LANGGRAPH=true`，将原有逻辑按 `Planner Agent -> Executor Agent -> Verifier Agent` 显式编排；三个角色仍复用同一套 QAEngine/ToolRegistry/Verifier。
...
- 已提供可选 JWT + Workspace user/group ACL；生产环境可由 API Gateway、企业 OIDC/SSO 签发身份，并继续配合网络隔离与审计日志。
- 报告 HTML 使用受控转义和 CSP；上传资料与研究数据库默认排除在 Git 和 Docker 构建上下文之外。
- Workspace ACL 负责资料、问答与研究任务隔离；认证默认关闭，不影响开源本地演示。
